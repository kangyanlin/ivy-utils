...
func (in *ReportGenerator) QualifyReport(combinedData map[string][]byte, xlsx bool, output string) (err error) {
	...
	if xlsx {
		...
		for num, qr := range result {
			lineHeight := 1
			buf.SetCellInt(DEF_XLSX_SHEET, axisFactory('A', row), num+1)
			buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('B', row), qr.Name)
			...
			buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('J', row), qr.RackName)
			buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('K', row), qr.RackSlot)
			buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('L', row), qr.CPUModel)
			buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('M', row), qr.CPUBaseFreq)
			buf.SetCellValue(DEF_XLSX_SHEET, axisFactory('N', row), qr.CPUCount)
			buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('O', row), fmt.Sprintf("%d / %d", qr.CPUCores, qr.CPUThreads))
			buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('P', row), fmt.Sprintf("%d / %d", qr.PopulatedDIMMs, qr.MaximumDIMMs))
			buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('Q', row), qr.InstalledMemory)
			if h := len(qr.VirtualDisks); h > lineHeight {
				lineHeight = h
			}
			for vdi := range qr.VirtualDisks {
				buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('R', row+vdi), qr.VirtualDisks[vdi].Name)
				buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('S', row+vdi), qr.VirtualDisks[vdi].Size)
			}
			buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('T', row), qr.PrimaryIPAddress)
			buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('U', row), qr.IPMIAddress)
			var h int
			for lii := range qr.LogicalInterfaces {
				...
				buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('V', row+h), qr.LogicalInterfaces[lii].Name)
				for limi := range qr.LogicalInterfaces[lii].Members {
					buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('W', row+h+limi), qr.LogicalInterfaces[lii].Members[limi].Name)
					buf.SetCellStr(DEF_XLSX_SHEET, axisFactory('X', row+h+limi), qr.LogicalInterfaces[lii].Members[limi].MACAddress)
				}
				h += length
			}
			if h > lineHeight {
